...
class TestGetPlanetPosition:
    def test_jupiter_returns_tuple(self):
        ...
        result = PlanetaryEphemerides.get_planet_position("Jupiter", now, 44.15, 5.23)
    ...
    def test_mars(self):
        now = datetime(2025, 6, 15, 22, 0, 0)
        result = PlanetaryEphemerides.get_planet_position("Mars", now, 44.15, 5.23)
        assert result is not None

    def test_lune(self):
        now = datetime(2025, 6, 15, 22, 0, 0)
        result = PlanetaryEphemerides.get_planet_position("Lune", now, 44.15, 5.23)
        assert result is not None

    def test_unknown_planet(self):
        now = datetime(2025, 6, 15, 22, 0, 0)
        result = PlanetaryEphemerides.get_planet_position("Pluton", now, 44.15, 5.23)
        assert result is None

    def test_french_name(self):
        """Nom français → fonctionne."""
        now = datetime(2025, 6, 15, 22, 0, 0)
        result = PlanetaryEphemerides.get_planet_position("Saturne", now, 44.15, 5.23)
        assert result is not None
