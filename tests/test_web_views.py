...
class TestParkView:
    def test_park(self, api_client, mock_ipc):
        response = api_client.post("/api/hardware/park/")
        assert response.status_code == 200
        assert "Parking" in response.data["message"]


class TestCalibrateView:
    def test_calibrate(self, api_client, mock_ipc):
        response = api_client.post("/api/hardware/calibrate/")
        assert response.status_code == 200
        assert "Calibration" in response.data["message"]


class TestEndSessionView:
    def test_end_session(self, api_client, mock_ipc):
